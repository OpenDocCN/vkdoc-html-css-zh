...
![images](images/square.jpg)T5 注第十一章中的`XMLHttpRequest`对象已涵盖。出于本章的目的，知道`XMLHttpRequest`是一个允许您向服务器端资源发出请求(GET/POST)的对象就足够了。不使用`$.ajax()`方法，你可以使用`XMLHttpRequest`对象来调用 web 方法、MVC 动作方法和通用处理程序。
