...
3.  Give it a one-pixel grey border through CSS, as shown in [Figure 6-1](#fig_6_1). `<canvas id="myCanvas" width="300" height="200" style="border:solid 1px #ccc;"></canvas>` ![images](images/0601.jpg)

    **图 6-1。**带有单像素边框的普通画布

    在你画之前，画布是一个普通的区域，但是你可以像在任何其他 HTML 元素上一样使用 CSS 属性，它会应用背景颜色、图像、笔触等等。

4.  Now you can add fallback content that you might want to show for non-supported browsers. An example of fallback content where canvas is not browser-supported is shown in [Figure 6-2](#fig_6_2). `<canvas id="myCanvas" width="300" height="200" stye="border:solid 1px #ccc;">
    Your browser doesn't support the HTML5 canvas element.
    <img src="fallbackPicture.jpg" alt="fallback picture"/>
    </canvas>` ![images](images/0602.jpg)
...
*   *Line join style.* If you want to draw a shape with several subpaths, there is a line property that addresses the rendering of the line connections: `lineJoin`. Paths can have three possible line joins: `miter`, `round`, or `bevel`, as shown in [Figure 6-8](#fig_6_8). By default, it's set to `miter`. ![images](images/0608.jpg)

    **图 6-8** 线连接

*   *Line width style.* You can also set the width of your line with the `lineWidth` property. The size should be given in pixels. By default, it's set to 1, and shown in [Figure 6-9](#fig_6_9). ![images](images/0609.jpg)
...
5.  You can now close your script tag. `</script>`
    ...
    `<body onload=drawShapes()>` ![images](images/0615.jpg)
...
`var myGradient=context.createLinearGradient(lineStartingPointX, lineStartingPointY,![images](images/U002.jpg)
 lineEndingPointX, lineEndingPointY);` ![images](images/0616.jpg)
...
`var gradient=context.createRadialGradient(startCircleX, startCircleY, startCirlceRadius,![images](images/U002.jpg)
...
`www.whatwg.org/specs/web-apps/current-work/multipage/the-canvas-element.html#dom-context-2d![images](images/U002.jpg)
-textbaseline`
