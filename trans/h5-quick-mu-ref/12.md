...
`标签指定 HTML5 文档、网站和应用程序中的键盘输入(或 HTML5 的电传输入，不再支持`标签)。一个文档中可以使用多个`标签。这个`标签使用下面的样式定义；如果您愿意，可以将其更改为在外部 CSS 样式表中指定 Courier 字体:````
...
`tag is usually nested inside a `tag, as variables exist inside code. Let’s use the tag to style variables in the `tag example in the following markup:```
