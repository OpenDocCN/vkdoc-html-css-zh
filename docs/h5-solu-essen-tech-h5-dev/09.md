...
如果连接期间没有发生错误，它将执行 open 事件，此时您的应用程序准备好使用以下语法向服务器发送数据:
...
                document.getElementById ('socketBtn').addEventListener('click',![images](images/U002.jpg)
...
         ws.onerror = function(evt)
         {
                 // check to see if the readyState attribute's value is (1) OPEN or![images](images/U002.jpg)
...
**图 9-4** 。只需将 XAMPP·DMG 的文件拖到 Mac 上的应用程序文件夹中就可以安装服务器了。
...
                document.getElementById ('messageBtn').addEventListener('click',![images](images/U002.jpg)
 sendMsg,true);
...
         ws.onerror = function(evt)
         {
                 //  check to see if the readyState attribute's value is OPEN (1) or![images](images/U002.jpg)
...
<input type="text" value="Insert a message to send" id="message" /> <button id=![images](images/U002.jpg)
...
`<input type="text" value="Insert a message to send" id="message" onkeypress=![images](images/U002.jpg)
...
`javascript:alert("This browser does " + (window.WebSocket ? "" : "not ") + "support![images](images/U002.jpg)
 WebSocket API");`
