...
这些模式、词汇和本体只是无尽变化的示范。这份名单无论如何都不完整。Schemaweb.info [ [2](#ref2) ]、vocab.org[[3](#ref3)]和其他库收集更具体和复杂的词汇、模式和本体，例如啤酒本体[ [4](#ref4) ]、音乐词汇[ [5](#ref5) ]或视频本体[ [6](#ref6) ]。你也可以用谷歌搜索找到很多词汇。

然而，网络本体论有其局限性和开放性的问题。例如，用规则扩展验证 OWL 本体是相当复杂的。但是，可以通过使用声明性方法(一种称为 Datalog 的新语言)[ [7](#ref7) ]在符号级别验证它们。应该解决各种本体之间的语义差异。实现这一目标的努力被称为*本体对齐*。从大型 RDFs 中存储和检索数据只能通过高级技术来执行[ [8](#ref8) ]。
...
微格式可以用`class`、`rel`和`rev`属性值[ [9](#ref9) ]来表达站点结构、链接权重、内容类型和人际关系。它们非常容易编写，支持它们的软件数量也在增加(例如，Operator [0]和 Tails Export[[1](#ref1)]Firefox 的附加组件，Google Chrome 扩展 Michromeformats [ [2](#ref2) ]，Microformats transformer Optimus[[3](#ref3)]，或者 Safari、Firefox 和 IE 的 Microformats Bookmarklet[[4](#ref4)])。

然而，仍有一些未决问题。例如，应该避免在`a`元素上应用各种微格式作为多个值(例如，`rel="nofollow"`和`rel="friend"`)。投票链接微格式使用的`rev`属性不能在 HTML5 中使用。

由`profile`属性提供的配置文件 URIs 不能在 HTML5 中的`head`元素上使用，其中`profile`属性值可以在锚(`a`或链接元素(`link`)上为`rel`属性声明。例如，为 hCalender 微格式提供了一个包含所有三个选项的配置文件 URI。hCalendar 微格式基于 iCalendar 标准(RFC 2445 [ [5](#ref5) ])。所有使用 hCalendar 符号的内容都应该引用 hCalendar XMDP 配置文件，换句话说就是 `[http://microformats.org/profile/hcalendar](http://microformats.org/profile/hcalendar)`，如[清单 7-7](#list_7_7) 或[清单 7-8](#list_7_8) 中所示，作为文档头，或[清单 7-9](#list_7_9) 中所示，作为文档体的一部分。这些方法也可以结合使用。
...
这些属性由 hCalendar 的元素表示。需要的属性有`dtstart`，需要以 ISO 日期格式提供， <sup>[2](#CHP-7-FN-2)</sup> 和`summary`。
...
  <div class="description">The Semantic Web Conference 2011 will take place in Pretoria, ![images](images/U001.jpg)
   South Africa.</div>
...
  <div>Dates: <abbr class="dtstart" title="20111012T093000Z">October 12, 2011, 09:30 ![images](images/U001.jpg)
   UTC</abbr> – <abbr class="dtend" title="20111014T200000Z">October 14, 2011, 20:00 ![images](images/U001.jpg)
   UTC</abbr></div>
...
第一个 HTML 微格式 XHTML 朋友网络(XFN)于 2003 年 12 月推出。XFN 是由全球多媒体协议组织设计的，用简单的超链接[ [9](#ref9) ]来表达人际关系。XFN 对于小册子风格的主页和博客条目特别有用。
...
用于解析相对 URIs 的基本 URIs 可以用形式为`@base <[http://example.com/overview/](http://example.com/overview/)>`的`@base`指令来设置。
...
正如前面所讨论的，微格式重用 HTML 属性(例如，`class`，`title`)，并且具有难以组合的独立词汇表，因为微格式不使用名称空间机制。最后但同样重要的是，微格式没有定义 RDF 表示。 <sup>[8](#CHP-7-FN-8)</sup>
...
按照 W3C 元数据活动的说法， *RDF Schema* ( *RDFS* )是“一种受来自知识表示的思想影响的陈述性表示语言” [4](#ref4) 。RDF Schema 用结构(类、属性的属性等等)扩展了 RDF。它可以用来形式化人类可读和机器可处理的词汇表之间的元数据交换。除了前面讨论的基本 RDF 词汇，RDFS 还有几个额外的构造:
...
<rdf:RDF ![images](images/U001.jpg)
 xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" ![images](images/U001.jpg)
 xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#" ![images](images/U001.jpg)
...
这个符号也可以用`rdfs:Class`代替`rdf:Description`并省略`rdf:type` ( [清单 7-70](#list_7_70) )来缩短。
...
*Web 本体语言*是一种知识表示语言，主要目的是创建 *web 本体*。Web 本体可以用于各种目的，例如搜索、查询形成、索引、代理或服务元数据管理，或者提高应用程序和数据库的互操作性。Web 本体对于知识密集型应用尤其有用，在这些应用中，文本提取、决策支持或资源规划是常见的任务，在用于知识获取的知识库中也是如此。

Web 本体语言的缩写 OWL 不是故意直白的[ [6](#ref6) ]。OWL 本体是 RDF 图，换句话说，是 RDF 三元组的集合。类似于 RDF 图，OWL 本体图可以用各种语法符号表示。猫头鹰有三个变种:*猫头鹰 Lite* 、*猫头鹰 DL* 、*猫头鹰 Full* [ [7](#ref7) 。OWL 是比 RDF 更高级的语言；实际上，它是 RDF 的一个词汇扩展。因此，RDF 图是完整的本体。

默认的 OWL 名称空间是`[http://www.w3.org/2002/07/owl#](http://www.w3.org/2002/07/owl)`，它定义了 OWL 词汇表。没有专门为 OWL 定义的 MIME 类型。建议对 OWL 文档使用`application/rdf+xml`或 `application/xml` MIME 类型。推荐的文件扩展名是`.rdf`或`.owl`。

OWL 的第一个版本开始于 2002 年，第二个版本 OWL2 开始于 2008 年。OWL 在 2004 年成为 W3C 推荐标准[8](#ref8)，OWL2 在 2009 年成为 W3C 推荐标准[149，150]。

##### 语法

在高层，可以使用 OWL 抽象语法[ [1](#ref1) ]和 OWL2 函数语法[ [2](#ref2) ]。OWL 还支持几种交换语法，包括 RDF 语法[ [3](#ref3) ] (RDF/XML [ [4](#ref4) ]，RDF/Turtle [ [5](#ref5) ])，OWL2 XML 语法[ [6](#ref6) ]，以及 Manchester 语法[ [7](#ref7) ]。RDF/XML 是标准语法[ [8](#ref8) ]。
...
属性特征由属性公理定义。基本形式只表达存在。例如，在智能手机本体中，属性`hasKeyboard`可以被声明来表达手机的一个主要特征(见[清单 7-76](#list_7_76) )。大多数入门级手机都有电话键盘，而许多智能手机只有触摸屏，但有些有完整的 QWERTY 键盘。

***清单 7-76。**猫头鹰里的一份财产申报*

`<owl:ObjectProperty rdf:ID="hasKeyboard"/>`

OWL 属性公理也可以定义额外的特征。OWL 支持 RDF 模式结构，如`rdfs:subPropertyOf`、`rdfs:domain`和`rdfs:range`。与其他属性的关系可以用`owl:equivalentProperty`和`owl:inverseOf` ( [列表 7-77](#list_7_77) )来表示。
...
全局基数约束由`owl:FunctionalProperty`和`owl:InverseFunctionalProperty`定义。逻辑属性特征由`owl:SymmetricProperty`和`owl:TransitiveProperty` [ [9](#ref9) ] ( [清单 7-78](#list_7_78) )定义。
...
OWL 为表达关系提供了精确的声明，即使它们是显而易见的。例如，两个智能手机特性的*属性层次*可以用函数语法来表达，如[清单 7-79](#list_7_79) 所示。
...
*   Class identifier (URI reference). A named instance of `owl:Class`, a subclass of `rdfs:Class`.<sup>[9](#CHP-7-FN-1)</sup> [Listing 7-80](#list_7_80) shows an example.

    ***清单 7-80。**OWL 中的一个类标识符*

    `<owl:Class rdf:ID="Handheld"/>`
*   Set of individuals (instances of a class) defined by the `owl:oneOf` property. For example, the class of smartphones can be declared in the RDF/XML syntax with the RDF construct `rdf:parseType="Collection"`, as shown in [Listing 7-81](#list_7_81).
...
*   Property restriction: a value constraint or a cardinality constraint (for example, [Listing 7-82](#list_7_82)).
...
*   Intersection of two or more class descriptions. For example, the intersection of two `Brochures` enumerations can be described by the statement `owl:intersectionOf`, as presented in [Listing 7-83](#list_7_83).
...
类描述可以组合成*类公理*。类的层次结构可以用*子类公理* ( [清单 7-84](#list_7_84) )来表示。
...
虽然一般来说，个人可以是几个阶层的成员，但在许多情况下，成员资格是排他性的。例如，智能手机属于直板或滑板外形。这种*级不衔接*可以用[清单 7-86](#list_7_86) 表示。
...
xmlns:air="http://www.daml.org/2001/10/html/airport-ont#" ![images](images/U001.jpg)
 xmlns:bio="http://vocab.org/bio/0.1/" ![images](images/U001.jpg)
 xmlns:bibo="http://purl.org/ontology/bibo/" ![images](images/U001.jpg)
 xmlns:cc="http://creativecommons.org/ns#" ![images](images/U001.jpg)
 xmlns:contact="http://www.w3.org/2000/10/swap/pim/contact#" ![images](images/U001.jpg)
 xmlns:dbp="http://dbpedia.org/property/" ![images](images/U001.jpg)
 xmlns:dbr="http://dbpedia.org/resource/" ![images](images/U001.jpg)
 xmlns:dc="http://purl.org/dc/elements/1.1/" ![images](images/U001.jpg)
 xmlns:dcterms="http://purl.org/dc/terms/" ![images](images/U001.jpg)
 xmlns:fb="http://www.facebook.com/2008/fbml" ![images](images/U001.jpg)
 xmlns:foaf="http://xmlns.com/foaf/0.1/" ![images](images/U001.jpg)
 xmlns:geo="http://www.w3.org/2003/01/geo/wgs84_pos#" ![images](images/U001.jpg)
 xmlns:ical="http://www.w3.org/2002/12/cal/icaltzd#" ![images](images/U001.jpg)
 xmlns:og="http://opengraphprotocol.org/schema/" ![images](images/U001.jpg)
 xmlns:openid="http://xmlns.openid.net/auth#" ![images](images/U001.jpg)
 xmlns:owl="http://www.w3.org/2002/07/owl#" ![images](images/U001.jpg)
 xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" ![images](images/U001.jpg)
 xmlns:rdfa="http://www.w3.org/ns/rdfa#" ![images](images/U001.jpg)
 xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#" ![images](images/U001.jpg)
 xmlns:rel="http://vocab.org/relationship/" ![images](images/U001.jpg)
 xmlns:rss="http://web.resource.org/rss/1.0/" ![images](images/U001.jpg)
 xmlns:sioc="http://rdfs.org/sioc/ns#" ![images](images/U001.jpg)
 xmlns:smap="http://purl.org/net/ns/sitemap#" ![images](images/U001.jpg)
 xmlns:vcard="http://www.w3.org/2006/vcard/ns#" ![images](images/U001.jpg)
 xmlns:wot="http://xmlns.com/wot/0.1/" ![images](images/U001.jpg)
 xmlns:xhv="http://www.w3.org/1999/xhtml/vocab#" ![images](images/U001.jpg)
 xmlns:xsd="http://www.w3.org/2001/XMLSchema#">`

这些*词汇前缀*允许使用当前文档中列出的外部词汇定义的术语。强烈建议只列出文档中实际使用的名称空间，这有助于代码优化和维护。尽管不必要的名称空间不会在浏览器中导致任何错误信息，但它们会使标记更加复杂。

由于词汇表的不同特性，组合应用程序并不总是简单明了的。例如，许多术语包含在各种词汇表中，选择其中一个往往只受个人偏好的驱使。事实上，您应该考虑候选属性的规范，以便为给定的场景选择最合适的属性。

#### 在 RDF 中组合词汇

混合任何词汇表中的属性的最简单方法是创建一个 RDF 文件。例如，一个完全机器可读的个人资料使用 FOAF 术语来描述一个人的头衔、姓名、性别、即时消息帐户、主页、电话号码和关系。离此人办公室最近的机场可以用联系词汇表中的一个术语来表示，文件创建的日期可以由 Dublin Core 来声明，等等([清单 7-88](#list_7_88) )。
...
<rdf:RDF ![images](images/U001.jpg)
 xmlns:contact="http://www.w3.org/2000/10/swap/pim/contact#" ![images](images/U001.jpg)
 xmlns:dc="http://purl.org/dc/terms/" ![images](images/U001.jpg)
 xmlns:foaf="http://xmlns.com/foaf/0.1/" ![images](images/U001.jpg)
 xmlns:google="http://rdf.data-vocabulary.org/#" ![images](images/U001.jpg)
 xmlns:owl="http://www.w3.org/2002/07/owl#" ![images](images/U001.jpg)
 xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" ![images](images/U001.jpg)
 xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#" ![images](images/U001.jpg)
 xmlns:wot="http://xmlns.com/wot/0.1/" ![images](images/U001.jpg)
...
如前所述，重叠特征可以用微数据和微格式来描述，也可以用 RDFa 来描述任意特征。微格式也可以与(X)HTML5 微数据同时使用。例如，hCard 可以与微数据同时应用，如[清单 7-89](#list_7_89) 所示。

***清单 7-89。**将 hCard 与微数据相结合*

`<dl class="vcard" itemscope itemtype="http://data-vocabulary.org/Person">
  <dt class="fn" itemprop="name"><a href="http://example.com" ![images](images/U001.jpg)
   itemprop="url">John Smith</a></dt>
  <dd class="title" itemprop="title">Photographer</dd>
  <dd class="adr" itemprop="address" itemscope ![images](images/U001.jpg)
   itemtype="http://data-vocabulary.org/Address"><span class="locality" ![images](images/U001.jpg)
   itemprop="locality">Memphis</span>, <abbr title="Tennessee" class="region" ![images](images/U001.jpg)
...
资源描述框架的灵活性使得同时使用来自外部名称空间的多种元数据变得很简单。因此，丰富的语义可以同时从各种词汇表添加到文档中([清单 7-90](#list_7_90) )。
...
当使用 XML 或 RDF/XML 语法时，Dublin Core 可以与外部元数据词汇表结合使用，如 IEEE 学习对象元数据(IMS) [ [1](#ref1) ]或开放数字版权语言(ODRL)元数据[ [2](#ref2) ]。例如，当使用绿石数字图书馆软件[ [3](#ref3) ]时，可以通过结合 Dublin Core、LOM 和描述学习材料的本地标签来适当地提供大学中数字学习材料的存储、保存和检索。

例如，一个在线教育系统可以用三种不同类型的元数据来精确描述([清单 7-92](#list_7_92) )。

***清单 7-92。**DC、IMS 和 ODRL 的组合*

`<record ![images](images/U001.jpg)
  ![images](images/U001.jpg)
 xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" ![images](images/U001.jpg)
 xsi:schemaLocation="http://www.example.org/virtualtutor/ ![images](images/U001.jpg)
    http://www.example.org/virtualtutor/schema.xsd" ![images](images/U001.jpg)
 xmlns:dc="http://purl.org/dc/elements/1.1/" ![images](images/U001.jpg)
 xmlns:dcterms="http://purl.org/dc/terms/" ![images](images/U001.jpg)
 xmlns:oex="http://odrl.net/1.0/ODRL-EX" ![images](images/U001.jpg)
 xmlns:odd="http://odrl.net/1.0/ODRL-DD" ![images](images/U001.jpg)
...
一种高级方法是通过标记提供图像元数据。alt 属性对于有效性至关重要。正确提供的属性值也可以提高可访问性(见第 10 章)。然而,`longdesc`属性可以用来提供到图像和其他非文本内容的详细描述的链接，它的软件支持很差，在(X)HTML5 中已经过时。

除了标记中提供的图像描述和前面讨论的用 RDFa 编写的描述或图像许可，高级和专业的图像处理软件，如 Adobe Photoshop，可用于添加或修改图像文件本身包含的元数据([图 7-4](#fig_7_4) )。在一定程度上，现代操作系统的文件浏览器也可以执行这项任务。

根据图像格式，可以应用各种各样的元数据，包括但不限于标题、作者、版权所有者、关键词、方向、视点、色彩空间、摄影师的联系数据、相机数据、来源、高级元数据等等。

![images](images/square.jpg) **注**关于博览会的几种类型的元数据被嵌入数码相机拍摄的 JPEG 文件中，这些文件可以在图像浏览器和处理应用程序中检索。

![images](images/0704.jpg)

***图 7-4。**在 Photoshop 中处理图像文件元数据*

在幕后，Adobe Photoshop 应用由 Adobe Systems 创建的可扩展元数据平台(XMP)标准。《XMP》的连载可以嵌入几种流行的文件格式，包括 GIF、JPEG、JPEG 2000、PNG、TIFF 图像，以及 PDF 文件 <sup>[10](#CHP-7-FN-10)</sup> [ [4](#ref4) 。GIF 支持 XMP 嵌入作为一个*应用扩展*，带有标识符 XMP 数据和认证码 XMP。在 JPEG 文件中，XMP 元数据嵌入在具有段头 http://ns.adobe.com/xap/1.0/\x00.的*应用段 1* (0xFFE1)中。在 JPEG 2000 图像中，XMP 元数据可以位于由 UID 0x be 7 ACF CB 97 a 942 e 89 c 71999491 E3 fac 标识的 uuid 原子处。在 PNG 文件中，xmp 嵌入在带有关键字 XML:com.adobe.xmp 的 iTXt 文本块中。XMP 位于 TIFF 图像中的*标签 700* 处。可移植文档格式支持在 PDF 对象的元数据流中嵌入 XMP。

![images](images/0705.jpg)

***图 7-5。**二进制 JPEG 图像文件中的文本 RDF 元数据*

从这个角度来看，图像的主要问题是它们是二进制文件([图 7-5](#fig_7_5) )，需要不同于文本文件的索引或搜索方法。例如，在*谷歌图片搜索*中，还有其他方面，包括用于消除非常小(例如，1x1 像素)的图片、骗局等的技术。在存储在图像文件本身中的元数据之上使用的文本元数据总是提供高级别的处理和共享选项。
...
关于如何通过删除`embed`标签并将所需属性移动到`object`标签来从 YouTube 嵌入代码中创建符合标准的标记的细节将在稍后的[第 13 章](13.html#ch13)中描述。

有两种视频元数据格式可以写入谷歌认可的标记中:脸书共享和雅虎！SearchMonkey RDFa [ [5](#ref5) ]。它们可以交替使用，也可以同时使用。它们应该以不需要 JavaScript 或 Flash 就可以阅读的方式直接写入标记中。
...
<meta name="description" content="Organist John Smith plays Praeludium and Fuge in A minor ![images](images/U001.jpg)
 by J. S. Bach" />
...
雅虎！SearchMonkey 元数据可以在`object`标签上提供，如[清单 7-94](#list_7_94) 所示。

***清单 7-94。**一个雅虎！SearchMonkey 示例*

`<object type="application/x-shockwave-flash" width="480" height="385" ![images](images/U001.jpg)
 data="http://www.youtube.com/v/a38-oj8VEXI&amp;hl=en_US&amp;fs=1&amp;" rel="media:video" ![images](images/U001.jpg)
** resource="http://www.youtube.com/v/a38-oj8VEXI&amp;hl=en_US&amp;fs=1&amp;" ![images](images/U001.jpg)**
** xmlns:media="http://search.yahoo.com/searchmonkey/media/" ![images](images/U001.jpg)**
...
所有其他标签都是可选的，包括都柏林核心名称空间(`xmlns:dc`)和都柏林核心元数据(`dc:contributor`、`dc:creator`、`dc:date`、`dc:description`、`dc:identifier`、`dc:license`、`dc:subject`)、媒体元数据(`media:duration`、`media:height`、`media:player`、`media:region`、`media:title`、`media:type`、`media:views`、`media:width`)以及`review:rating`、 [6](#ref6) 。

### SEO 中的元数据

虽然较高的页面排名或较好的搜索列表位置与标准化网站之间没有严格的相关性，但有几种技术有助于获得更好、更高质量的 web 文档。语义内容为 web 文档增加了意义，使信息索引、搜索和处理更加容易。元数据注释极大地扩展了 web 搜索的潜力。

RDFa 丰富的 web 文档的潜力正在增加，因为主要的搜索引擎已经开始在索引的同时处理这些注释。越来越多的元数据被搜索引擎索引和考虑。例如，谷歌在 2009 年开始在搜索结果页面上应用 hCard、hProduct 和 hReview 微格式以及 RDFa，称之为*丰富片段* [ [7](#ref7) 。雅虎！自 2008 年以来，已经为 RDFa 和微格式编制了索引[ [8](#ref8) ]。富含 RDFa 或微格式的 Web 站点内容可以通过提供有用的结构化数据来增强 web 搜索。

Google 对 hCard 微格式派生的微数据、微格式和 RDFa contact 属性进行索引，包括`name`、`nickname`、`photo`、`title`、`role`、`url`、`affiliation`、`friend`、`contact`、`acquaintance`和`address`。分别代表名称、隶属关系和地址的微格式属性`fn`、`org`和`adr`也显示在 web 搜索页面上。 <sup>[11](#CHP-7-FN-11)</sup> 此外，谷歌还能识别 XFN 微格式`friend`、`contact`和`acquaintance` [0】。在 XFN 和 FOAF 表达的关系和联系也由谷歌[ [1](#ref1) ]处理。

如前所述，锚元素上的`rel`属性的`nofollow`属性值可用于确保网页不会从目的链接获得 PageRank。这样的链接被 Google 关注，但是不添加 PageRank。Bing 不跟随链接，页面也不被索引。雅虎！跟随链接并索引页面。[清单 7-95](#list_7_95) 显示了一个例子。

***清单 7-95。**在链接上应用`nofollow`属性*

`<a href="http://www.lowprsite.com" rel="nofollow">External site</a>`

雅虎！SearchMonkey 支持以下元数据[ [2](#ref2) ]:
...
雅虎中的微格式搜索可以用清单 7-96 中的[所示的形式的查询很容易地执行。](#list_7_96)
...
RDFa 之类的元数据注释可以有效地提供更好的搜索结果。网上提供的语义内容越多，从搜索引擎得到的搜索结果就越合理和相关。 <sup>[12](#CHP-7-FN-12) 用(X)HTML 标记、XML、RDF、FOAF 和 DOAC 描述的小册子式名片和个人信息的索引非常简单。然而，嵌入在传统标记中的语义内容只有在网络爬虫使用的机制支持时才能被处理。幸运的是，可由搜索引擎处理的有意义的元数据类型的数量在不断增加。</sup>
...
元数据嵌入与可访问性密切相关。可访问性指南可以确保为对象提供替代内容，并且文档结构组织良好([第 10 章](10.html#ch10))。

同样需要注意的是，SEO 和用户体验(UX)并不一定一致。例如，频繁重复的关键字会降低人的可读性[ [3](#ref3) ]。
...
7.  Murphy T (2010) Lin Clark 阐述了 Drupal 的重要性。社交媒体。[http://social media . net/2010/09/07/Lin-Clark-on-why-Drupal-matters](http://socialmedia.net/2010/09/07/lin-clark-on-why-drupal-matters)。2010 年 9 月 9 日访问
...
9.  科比·N(编辑)(2011)问&答:康拉德·沃尔夫拉姆谈 Web 3.0 中的应用程序交流。丹尼斯出版有限公司[www . it pro . co . uk/621535/q-a-Conrad-wolfram-on-communication-with-apps-in-web-3-0](http://www.itpro.co.uk/621535/q-a-conrad-wolfram-on-communicating-with-apps-in-web-3-0)。2011 年 8 月 25 日访问
...
15.  Clark K(2010)RDF 数据库市场。克拉克&帕西亚有限责任公司。[http://weblog . clarkparsia . com/2010/09/23/the-rdf-database-market/](http://weblog.clarkparsia.com/2010/09/23/the-rdf-database-market/)。2010 年 11 月 21 日访问
...
22.  雅虎(2010)Flickr——照片分享。雅虎！www.flickr.com 公司[【T2 公司】。2010 年 10 月 15 日访问](http://www.flickr.com)
23.  谷歌站长中心团队(2007)使用 robots meta 标签。谷歌公司[http://Google webmastercentral . blogspot . com/2007/03/using-robots-meta-tag . html](http://googlewebmastercentral.blogspot.com/2007/03/using-robots-meta-tag.html)。2010 年 10 月 16 日访问
...
26.  Sullivan D (2007)雅虎提供 NOYDIR 选择退出雅虎目录标题&描述。搜索引擎之地。第三门媒体。[http://search engine land . com/Yahoo-provides-noydir-opt-out-of-Yahoo-directory-titles-descriptions-10631](http://searchengineland.com/yahoo-provides-noydir-opt-out-of-yahoo-directory-titles-descriptions-10631)。2010 年 10 月 16 日访问
...
30.  Davis I、Newman R、D'Arcus B (2005)在 RDF 中表达核心 FRBR 概念。理查德.纽曼伊恩.戴维斯。[http://vocab.org/frbr/core.html](http://vocab.org/frbr/core.html)2011 年 2 月 21 日访问
31.  知识共享(2011)描述了 RDF 中的版权。知识共享权利表达语言。知识共享。[http://creativecommons.org/ns](http://creativecommons.org/ns)2011 年 2 月 21 日访问
32.  ODRL (2011)开放数字版权语言(ODRL)倡议。[http://odrl.net](http://odrl.net)2011 年 8 月 25 日访问
...
36.  Davis I，Galbraith D (2010) BIO:传记信息的词汇。伊恩·戴维斯，大卫·加尔布雷斯。[http://vocab.org/bio/0.1/](http://vocab.org/bio/0.1/)2011 年 2 月 21 日访问
37.  学习对象元数据的 IEEE 1484.12.1-2002 标准的 IMS (2006) IMS 元数据最佳实践指南。版本 1.3 最终规格。IMS 全球学习联盟。[http://www . IMS global . org/metadata/mdv1p 3/imsmd _ bestv1p 3 . html](http://www.imsglobal.org/metadata/mdv1p3/imsmd_bestv1p3.html)。2011 年 2 月 21 日访问
...
39.  Brickley D，Miller L (2007) FOAF 词汇规范 0.9。丹·布里克利，莉比·米勒。[http://xmlns.com/foaf/0.1/](http://xmlns.com/foaf/0.1/)2011 年 2 月 21 日访问
40.  帕拉达 RA (2008) DOAC 词汇规范。拉蒙·安东尼奥·帕拉达。[http://ramonantonio.net/doac/0.1/](http://ramonantonio.net/doac/0.1/)2011 年 2 月 21 日访问
41.  邓比尔·E(2011)DOAP。艾德·邓比尔。[http://trac.usefulinc.com/doap](http://trac.usefulinc.com/doap)2011 年 2 月 22 日访问
42.  林赛诉 Schemaweb 案(2011 年)。RDF 模式目录。VicSoft 有限公司 [www.schemaweb.info](http://www.schemaweb.info) 。2011 年 2 月 21 日访问
43.  戴维斯一世(2006)vocab.org——一个词汇的 URI 空间。伊恩·戴维斯。[http://vocab.org](http://vocab.org)2011 年 2 月 21 日访问
...
46.  Sikos LF(2011)vid ont–视频本体。[http://vidont.org/](http://vidont.org/)2011 年 5 月 30 日访问
...
50.  Kaply M (2010)火狐操作插件。迈克尔·卡普里。[https://addons.mozilla.org/en-US/firefox/addon/4106/](https://addons.mozilla.org/en-US/firefox/addon/4106/)2010 年 11 月 17 日访问
51.  De Bruin R (2010) Tails 出口火狐插件。罗伯特·德·布鲁因。[https://addons.mozilla.org/en-US/firefox/addon/2240/](https://addons.mozilla.org/en-US/firefox/addon/2240/)2010 年 11 月 17 日访问
52.  ryckbost b(2010)michrome 格式 Google Chrome 扩展。布莱恩·赖克博。[https://chrome . Google . com/extensions/detail/oalbifknmclbnmj jdemjkhmpjjj](https://chrome.google.com/extensions/detail/oalbifknmclbnmjlljdemhjjlkmppjjl)。2010 年 11 月 17 日加入
53.  Baranovskiy D(2010)Optimus–微格式转换器。德米特里·巴拉诺夫斯基。[http://microformatique.com/optimus/](http://microformatique.com/optimus/)2010 年 11 月 17 日访问
54.  左逻辑(2010)微格式书签。左逻辑有限公司[http://leftlogic.com/projects/microformats_bookmarklet](http://leftlogic.com/projects/microformats_bookmarklet)。2010 年 11 月 17 日访问
...
56.  elik T，Suda B (2010) hCalendar 1.0。微格式社区。[http://microformats.org/wiki/hcalendar](http://microformats.org/wiki/hcalendar)2010 年 11 月 12 日访问
57.  King R，elik T，Mullen weg M(2008)hCalendar Creator。微格式社区。[http://microformats.org/code/hcalendar/creator](http://microformats.org/code/hcalendar/creator)2010 年 11 月 11 日访问
58.  elik T，Suda B (2010) hCard 1.0。微格式社区。[http://microformats.org/wiki/hcard](http://microformats.org/wiki/hcard)2010 年 11 月 11 日访问
...
60.  elik T (2005) hCard 创建者。微格式社区。[http://microformats.org/code/hcard/creator](http://microformats.org/code/hcard/creator)2010 年 11 月 11 日访问
61.  Casserly C、Domicone A、Green L、Heung A、Kinkade N、Linksvayer M、Park J、Peters D、Rees J、Roberts A、Rose T、Ruttenberg A、Schultz AJ、Steuer E、Vollmer T、Webber C、Wilbanks J、Yergler N、Yip J 等人(编辑)(2010)执照。知识共享。[http://creativecommons.org/about/licenses/](http://creativecommons.org/about/licenses/)2010 年 10 月 17 日访问
62.  elik T (2005) rel= "许可证"。[http://microformats.org/wiki/rel-license](http://microformats.org/wiki/rel-license)微格式社区。2010 年 11 月 12 日访问
63.  Olbertz D(2010)No to rel = " No follow "。对抗垃圾邮件，而不是博客。德克·奥尔伯茨。[http://nonofollow.net](http://nonofollow.net)2010 年 11 月 13 日访问
64.  elik T，Marks K (eds)，Cutts M，Shellen J (2005) rel="nofollow "。微格式社区。[http://microformats.org/wiki/rel-nofollow](http://microformats.org/wiki/rel-nofollow)2010 年 11 月 12 日访问
65.  elik T，Marks K，Powazek D (2010) rel="tag "。微格式社区。[http://microformats.org/wiki/rel-tag](http://microformats.org/wiki/rel-tag)2010 年 11 月 12 日访问
...
67.  elik T，Marks K (2010)投票链接。微格式社区。[http://microformats.org/wiki/vote-links](http://microformats.org/wiki/vote-links)2010 年 11 月 13 日访问
68.  Paul A (2003)社交网络开始在网络上成形。西雅图时报。[http://community.seattletimes.nwsource.com/archive/?date = 2003 12 29&slug = Paul 29](http://community.seattletimes.nwsource.com/archive/?date=20031229&slug=paul29)。2010 年 11 月 10 日访问
69.  GMPG(2010)XFN-XHTML 朋友网。全球多媒体协议组。[http://gmpg.org/xfn/](http://gmpg.org/xfn/)2010 年 11 月 10 日访问
70.  Lewis EP (2008)用微格式获取语义，第 2 部分:XFN。艾米莉·p·刘易斯。[http://ablognotlimited . com/index . PHP/articles/getting-semantic-with-microformats-part-2-xfn/](http://ablognotlimited.com/index.php/articles/getting-semantic-with-microformats-part-2-xfn/)。2010 年 11 月 11 日访问
71.  Mullenweg M，elik T (2004) XFN 1.1 创建者。全球多媒体协议组。[http://gmpg.org/xfn/creator](http://gmpg.org/xfn/creator)2010 年 11 月 11 日访问
72.  mulweng m(2010)exefen。马修穆朗格。[http://ma . TT/tools/exefen . PHP/](http://ma.tt/tools/exefen.php/)。2010 年 11 月 11 日加入
73.  elik T (2003) XMDP:简介和格式描述。全球多媒体协议组。[http://gmpg.org/xmdp/description](http://gmpg.org/xmdp/description)2010 年 11 月 13 日访问
74.  elik T(2010)ADR 微格式草案规范。微格式社区。[http://microformats.org/wiki/adr](http://microformats.org/wiki/adr)2010 年 11 月 13 日访问
75.  elik T(2009)geo 微格式草案规范。微格式社区。[http://microformats.org/wiki/geo](http://microformats.org/wiki/geo)2010 年 11 月 13 日访问
76.  Janes D，Carlyle B，Tantek e lik T(2010)hAtom 微格式草案规范。微格式社区。[http://microformats.org/wiki/hatom](http://microformats.org/wiki/hatom)2010 年 11 月 13 日访问
77.  Sporny M，McEvoy M 等人(2010 年)提出的 hAudio 微格式草案规范。微格式社区。[http://microformats.org/wiki/haudio](http://microformats.org/wiki/haudio)2010 年 11 月 13 日访问
78.  埃利克·T、哈雷·R(编辑)、阿尔金·A、多纳托·C、金·R(2010)所列的缩微格式草案规范。微格式社区。[http://microformats.org/wiki/hlisting](http://microformats.org/wiki/hlisting)2010 年 11 月 13 日访问
79.  McEvoy M、elik T、Marks K、Hodder M、Begbie R、Kinberg J、Messina C、Rein L、Newell C、Sporny M、Johnson M、MC evoy M(2010)hMedia 微格式草案规范。微格式社区。[http://microformats.org/wiki/hmedia](http://microformats.org/wiki/hmedia)2010 年 11 月 13 日访问
80.  Malek J，Myles S，Moore M，Ng M，Martin TB(2010)hNews 微格式草案规范。微格式社区。[http://microformats.org/wiki/hnews](http://microformats.org/wiki/hnews)2010 年 11 月 13 日访问
81.  Lee P，Myers J，Cook C，gustaf son A(2010)h product 微格式草案规范。微格式社区。[http://microformats.org/wiki/hproduct](http://microformats.org/wiki/hproduct)2010 年 11 月 13 日访问
82.  l rtsch T(ed)，Berriman F，Ward B，Inkster T(2010)h recipe 微格式草案规范。微格式社区。[http://microformats.org/wiki/hrecipe](http://microformats.org/wiki/hrecipe)2010 年 11 月 13 日访问
83.  King R，elik T，Levine J，Marks K(2010)hResume 微格式草案规范。微格式社区。[http://microformats.org/wiki/hresume](http://microformats.org/wiki/hresume)2010 年 11 月 13 日访问
84.  elik T，Diab A，McAllister I，Panzer J，Rifkin A，Sippey M(2010)hre view 微格式草案规范。微格式社区。[http://microformats.org/wiki/hreview](http://microformats.org/wiki/hreview)2010 年 11 月 13 日访问
85.  King R，Cook B，elik T，Marks K(2009)rel-directory 微格式草案规范。微格式社区。[http://microformats.org/wiki/rel-directory](http://microformats.org/wiki/rel-directory)2010 年 11 月 14 日访问
86.  Marks K(2009)rel = " enclosure "微格式草案规范。微格式社区。[http://microformats.org/wiki/rel-enclosure](http://microformats.org/wiki/rel-enclosure)2010 年 11 月 14 日访问
87.  Ayers D(2009)rel = " home "微格式草案规范。微格式社区。[http://microformats.org/wiki/rel-home](http://microformats.org/wiki/rel-home)2010 年 11 月 14 日访问
88.  Pedersen AH、Kinberg J、Dedman J、Van Dijk P(2010)rel-payment 微格式草案规范。微格式社区。[http://microformats.org/wiki/rel-payment](http://microformats.org/wiki/rel-payment)2010 年 11 月 14 日访问
89.  Janes P (2009 年)机器人排斥概况。规范草案。微格式社区。[http://microformats.org/wiki/robots-exclusion](http://microformats.org/wiki/robots-exclusion)2010 年 11 月 14 日访问
90.  Gibson B (2010)的 xFolk 微格式。规范草案。微格式社区。[http://microformats.org/wiki/xfolk](http://microformats.org/wiki/xfolk)2010 年 11 月 14 日访问
91.  4K 协会(2009)所以你想开发一种新的微格式？微格式社区。[http://microformats.org/wiki/process](http://microformats.org/wiki/process)2010 年 11 月 13 日访问
92.  希克森一世(2010) HTML 微数据。万维网联盟。http://www.w3.org/TR/microdata/.于 2010 年 10 月 17 日加入
93.  Berners-Lee T (2007)巨型全球图。麻省理工学院。[http://dig.csail.mit.edu/breadcrumbs/node/215](http://dig.csail.mit.edu/breadcrumbs/node/215)2010 年 9 月 23 日访问
94.  Berners-Lee T (2007)巨型全球图。麻省理工学院。[http://dig.csail.mit.edu/breadcrumbs/node/215](http://dig.csail.mit.edu/breadcrumbs/node/215)2010 年 9 月 23 日访问
95.  Brickley D (2010) FOAF 项目网站。朋友的朋友(FOAF)项目。[www.foaf-project.org/](http://www.foaf-project.org/)2010 年 9 月 23 日访问
96.  QDOS (2010) FOAFNet。qdos！qdos！ [http://foaf.qdos.com/](http://foaf.qdos.com/) 。2010 年 9 月 23 日加入
...
101.  沃克 J (2010)的 Drupal FOAF 模块。[http://drupal.org/project/foaf](http://drupal.org/project/foaf)德赖斯·布塔尔特。2010 年 9 月 23 日访问
...
105.  希尔曼 D (2005)语法问题。In:使用都柏林核心。都柏林核心元数据倡议。[http://dublincore.org/documents/usageguide/#whichsyntax](http://dublincore.org/documents/usageguide/#whichsyntax)2010 年 11 月 2 日访问
106.  Powell A，Nilsson M，Naeve A，Johnston P，Baker T (2007) DCMI 抽象模型。DCMI 推荐。都柏林核心元数据倡议。[http://dublincore.org/documents/abstract-model/](http://dublincore.org/documents/abstract-model/)2010 年 10 月 25 日访问
107.  DCMI (2010)都柏林核心元数据元素集，版本 1.1。DCMI 推荐。都柏林核心元数据倡议。[http://dublincore.org/documents/dces/](http://dublincore.org/documents/dces/)2010 年 10 月 22 日访问
108.  Powell A、Wagner H (eds)、Weibel S、Baker T、Matola T、Miller E、Johnston P (2007)都柏林核心元数据倡议(DCMI)的名称空间政策。都柏林核心元数据倡议。[http://dublincore.org/documents/dcmi-namespace/](http://dublincore.org/documents/dcmi-namespace/)2010 年 10 月 26 日访问
109.  Powell A，Nilsson M，Naeve A，Johnston P，Baker T (2007) DCMI 抽象模型语义学。在:DCMI 抽象模型。都柏林核心元数据倡议。[http://Dublin core . org/documents/2007/02/05/abstract-model/# Sect-5](http://dublincore.org/documents/2007/02/05/abstract-model/#sect-5)。2010 年 10 月 26 日访问
110.  Hillmann D (2005)使用都柏林核心元素，都柏林核心元数据倡议。都柏林核心元数据倡议。[http://dublincore.org/documents/usageguide/elements.shtml](http://dublincore.org/documents/usageguide/elements.shtml)2010 年 9 月 11 日访问
111.  DCMI 使用委员会(2010) DCMI 元数据术语。DCMI 推荐。都柏林核心元数据倡议。[http://dublincore.org/documents/dcmi-terms/](http://dublincore.org/documents/dcmi-terms/)2010 年 10 月 22 日访问
112.  Berners-Lee T，Fielding R，Masinter L (2005) RFC 3986:统一资源标识符(URI):通用语法。互联网工程任务组。[http://www.ietf.org/rfc/rfc3986.txt](http://www.ietf.org/rfc/rfc3986.txt)2010 年 10 月 26 日访问
113.  Johnston P，Powell A (2008)使用 HTML/XHTML 元和链接元素表达都柏林核心元数据。都柏林核心元数据倡议。[http://dublincore.org/documents/dc-html/](http://dublincore.org/documents/dc-html/)2010 年 11 月 9 日访问
114.  Powell A，Johnston P (2003)用 XML 实现都柏林核心的指南。都柏林核心元数据倡议。[http://dublincore.org/documents/dc-xml-guidelines/](http://dublincore.org/documents/dc-xml-guidelines/)2010 年 11 月 6 日访问
115.  Johnston P，Powell A (2008)使用 XML (DC-DS-XML)表达都柏林核心描述集。都柏林核心元数据倡议。[http://dublincore.org/documents/dc-ds-xml/](http://dublincore.org/documents/dc-ds-xml/)2010 年 11 月 7 日访问
...
117.  Beckett D，Miller E，Brickley D (2002)用 RDF/XML 表达简单的都柏林核心。都柏林核心元数据倡议。[http://dublincore.org/documents/dcmes-xml/](http://dublincore.org/documents/dcmes-xml/)2010 年 10 月 31 日访问
118.  Kokkelink S，schwnzl R(2002)在 RDF / XML 中表达合格的都柏林核心。都柏林核心元数据倡议。[http://dublincore.org/documents/dcq-rdf-xml/](http://dublincore.org/documents/dcq-rdf-xml/)2010 年 11 月 2 日访问
119.  Nilsson M、Powell A、Johnston P、Naeve A (2008)使用资源描述框架(RDF)表达都柏林核心元数据。都柏林核心元数据倡议。[http://dublincore.org/documents/dc-rdf/](http://dublincore.org/documents/dc-rdf/)2010 年 11 月 2 日访问
120.  Nilsson M，Baker T (2008)对 RDF 中 Dublin Core 元数据的 DCMI 规范的注释。都柏林核心元数据倡议。[http://dublincore.org/documents/dc-rdf-notes/](http://dublincore.org/documents/dc-rdf-notes/)2010 年 11 月 2 日访问
...
128.  Chris Bizer C，cyganak r(2007)trig 语法。柏林自由大学。[http://www . wiwiss . fu-Berlin . de/bizer/trig/spec/](http://www4.wiwiss.fu-berlin.de/bizer/TriG/Spec/)。2010 年 11 月 21 日加入
...
136.  Beckett D (2011)雷德兰 RDF 库。戴夫·贝克特。[http://librdf.org](http://librdf.org)2011 年 8 月 26 日访问
...
160.  Kokkelink S，schwnzl R(2002)DC 与其他词汇和 DumbDown 合作。In:用 RDF / XML 表达合格的都柏林核心。都柏林核心元数据倡议。[http://dublincore.org/documents/dcq-rdf-xml/#sec3](http://dublincore.org/documents/dcq-rdf-xml/#sec3)2010 年 11 月 2 日访问
...
162.  Iannella R (ed) (2002) ODRL 1.1 表达式语言模式。开放数字权利语言倡议。[http://odrl.net/1.1/ODRL-EX-11-DOC/index.html](http://odrl.net/1.1/ODRL-EX-11-DOC/index.html)2010 年 10 月 23 日访问
...
164.  Adobe Systems Inc. (2010)在应用程序文件中嵌入 XMP 元数据。见:XMP 规范，第 3 部分-文件存储。[www . Adobe . com/content/dam/Adobe/en/devnet/xmp/pdf/xmpspecificationpart 3 . pdf](http://www.adobe.com/content/dam/Adobe/en/devnet/xmp/pdfs/XMPSpecificationPart3.pdf)。2010 年 11 月 17 日访问
165.  Sikos LF (2011)针对 Web 3.0 视频的高级(X)HTML5 元数据和语义。DESIDOC 图书馆与信息技术杂志 2011，31(4):247–252；[http://publications . drdo . gov . in/ojs/index . PHP/DJ lit/article/view file/1105/434](http://publications.drdo.gov.in/ojs/index.php/djlit/article/viewFile/1105/434)。2011 年 7 月 20 日访问
166.  雅虎！Inc .(2009)search monkey–视频。雅虎！开发者网络。http://developer.search.yahoo.com/help/objects/video 公司[【T2 公司】。2010 年 10 月 15 日访问](http://developer.search.yahoo.com/help/objects/video)
167.  Goel K，Guha RV，Hansson O (2009)介绍了丰富的片段。谷歌公司[http://Google webmastercentral . blogspot . com/2009/05/introducing-rich-snippets . html](http://googlewebmastercentral.blogspot.com/2009/05/introducing-rich-snippets.html)。2010 年 10 月 21 日访问
...
169.  Goel K，Gupta P，Hansson O (2009)帮助我们让网络更好:丰富片段的更新。谷歌公司[http://Google webmastercentral . blogspot . com/2009/10/help-us-make-web-better-update-on-rich . html](http://googlewebmastercentral.blogspot.com/2009/10/help-us-make-web-better-update-on-rich.html)。2010 年 10 月 21 日访问
...
171.  谷歌(2010 年)社交图谱 API。谷歌代码实验室。[http://code.google.com/intl/hu/apis/socialgraph/](http://code.google.com/intl/hu/apis/socialgraph/)2010 年 11 月 11 日访问
172.  雅虎！开发者网络(2008) Monkey 发现微格式和 RDF。雅虎！Inc .[http://developer . Yahoo . com/blogs/ydn/posts/2008/12/monkey _ finds _ microformats _ and _ rdf/](http://developer.yahoo.com/blogs/ydn/posts/2008/12/monkey_finds_microformats_and_rdf/)。2010 年 11 月 13 日访问
