...
脸书社交图是世界上最大的社交图，包含大约 10 亿用户的数十 Pb 的结构化数据。因为每个对象都是一个图节点，每个关系都是脸书社交图上的一个图边(见图 [8-4](#Fig4) )，任何对象都可以作为用户在浏览器中直接访问，也可以通过脸书应用以编程方式访问。
