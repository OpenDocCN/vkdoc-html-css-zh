...
![images](images/9781430242758_Fig17-01.jpg)
...
`<div id="button">
        <input type="button" **value="Print this page"** onclick="window.print()"![images](images/U002.jpg)
...
`        <div id=button>
        <input id="printbtn" type="button" value="Print this page" ![images](images/U002.jpg)
...
`        #button { text-align:center;
        }
        #printbtn { margin-bottom:10px; padding:5px; font-weight:bold; ![images](images/U002.jpg)
...
![images](images/9781430242758_Fig17-03.jpg)

***图 17-3** 。第一个打印页面*

![images](images/9781430242758_Fig17-04.jpg)
...
![imaages](images/square.jpg) **注意**打印的页面有一个小缺点:多栏页面打印每一栏，使其横跨页面的整个宽度，如图[图 17-3](#fig_17_3) 和 [17-4](#fig_17_4) 所示。
...
<img title="XXX free" src="images/xxx-crop.jpg" alt="XXX free" width="158" height="81">
...
某些版本的 Mozilla Firefox 有时会因为一个内置的怪癖而插入一个空白页。为了避免任何浏览器怪癖，用`re-set`开始样式表，如[清单 17-2b](#list_17_2b) 所示。
...
h1, h2, h3, h4, h5, h6 { margin-top:0; margin-bottom:0; font-weight:bold; ![images](images/U002.jpg)
...
![imaages](images/square.jpg) **注意**`print.css`样式表不包括任何已经被主(屏幕)样式表覆盖的命令。打印页面时，打印样式表会覆盖一些主样式表命令。
...
`body { background-color:#D7FFEB; font-family: "Times New Roman"; font-size: medium; ![images](images/U002.jpg)
color: #000000;
}
...
#hdr { background-position:35% 20%; background-image:url('images/compbkgcrop.jpg'); ![images](images/U002.jpg)
background-repeat:no-repeat; width:960px; height:170px;
}
#button {margin:auto; }
h1 { position:absolute; top:90px; left:300px; width:450px; font-family:"Times New Roman"; ![images](images/U002.jpg)
...
li.hbtnew {display:block; margin:auto; float:left; text-align:center; padding:3px; ![images](images/U002.jpg)
list-style-type:none; list-style-position:inherit;
}
li.hbtnew a {display:block; text-decoration:none; color:white; background-color:#0080a0; ![images](images/U002.jpg)
...
![images](images/9781430242758_Fig17-05.jpg)
...
![imaages](images/square.jpg) **注**本章描述的页面打印机并没有颠覆搜索引擎。使用本章所示的 CSS 页面打印机可以避免这个问题。

#### 测试 CSS 可打印页面

没有必要浪费纸张和墨水，即使测试需要合理的反复试验。将页面加载到浏览器中，然后单击文件![images](images/U001.jpg)打印预览，查看可打印页面的外观。如果可打印页面包含分页符，请单击打印预览屏幕底部的向右箭头查看后续页面。
...
![images](images/9781430242758_Fig17-06.jpg)
...
![imaages](images/square.jpg) **注**只有列出的三页才会有一个`DOCTYPE`。支持文件(`*.dat`和`count*.php`)必须*而不是*有一个`DOCTYPE`。
...
>     </html>`
>     
>     ![images](images/square.jpg)**注意**因为页面包含了一些 PHP，所以本例中的`charset`必须是`windows-1252`；否则,页面将无法验证.
>     
>     
...
![imates](images/square.jpg) **注** CHMOD 代表改变模式；也就是说，它使您能够更改文件的访问权限，从而使内容不能被 Linux 服务器上未经授权的人激活。主页面保留为 CHMOD 664，因此不会受到干扰。其他页面被设置为 777，以允许文件之间进行交互，从而在每次有人查看页面时改变访问者数量。在支持 PHP 的 Windows 服务器上，这要稍微复杂一些。最简单的方法是将`.dat`文件放在子目录中，并将子目录设置为 777。
...
![imaages](images/square.jpg) **注意**如果出现 PHP 解析错误，很可能是在`<?`和`php`之间插入了空格。删除那个空格，使它看起来像`<?php`。
...
在 Google Analytics 的左侧菜单中，选择流量来源![images](images/U001.jpg)关键字。这将向您显示人们为了到达您的网站而搜索的所有关键字的列表。
...
![images](images/9781430242758_Fig17-07.jpg)

***图 17-7。**默认链接重定向*

![images](images/9781430242758_Fig17-08.jpg)
...
**<p><b>Return to Menu</b><a href="#top"><img title="Return to Menu"** ![images](images/U002.jpg)
...
![images](images/9781430242758_Fig17-09.jpg)

***图 17-9。**重定向到另一个页面上的特定位置*

`page1.html`中的重定向链接将是`page2.html#tennis-prices`。完整的链接是:

`<b>For details of the cost of using the tennis courts</b> Please click ![images](images/U002.jpg)
<a href="page2.html#tennis-prices" title="Tennis court hire prices"><b>![images](images/U002.jpg)
Tennis prices</b </a>`
...
![images](images/9781430242758_Fig17-10.jpg)
...
        <style type="text/css">
        body {font-weight:bold; font-size: large; color:black; font-family:arial; ![images](images/U002.jpg)
        text-align:center; }
        </style>
</head>
<body>
...
<p>You will be switched to the new version at ![images](images/U002.jpg)
<a href="http://www.***brightville-council.org.uk***"> ![images](images/U002.jpg)
www.***brightville-council.org.uk*** </a> in 5 seconds</p>
<p>If re-direction does not occur after 5 seconds, please click ![images](images/U002.jpg)
...
![images](images/9781430242758_Fig17-11.jpg)
...
        <style type="text/css">
        #button {height:22px; width:125px; margin:auto; background-color:#6aaaff; ![images](images/U002.jpg)
        text-align:center;
        }
...
<body>
<div id="button">
        <a class="button" href="***previous-page*.html**" title="Return to previous screen ![images](images/U002.jpg)
...
![images](images/9781430242758_Fig17-12.jpg)
