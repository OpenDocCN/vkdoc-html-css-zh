...
![Image](img/square.jpg) **注意**即使实现得当，也要注意拖放在任何应用程序中的局限性。如果拖动行为被覆盖，使用拖动手势导航的移动设备可能无法正常工作。此外，拖放会干扰拖动选择。小心谨慎、适当地使用它。
...
![Image](img/square.jpg) **注意**微软在`[http://ie.microsoft.com/testdrive/HTML5/ComparingEventModels](http://ie.microsoft.com/testdrive/HTML5/ComparingEventModels)`提供了一个很棒的事件模型互动演示
...
现在，我们将根据事件处理程序通常触发的顺序，依次检查实际的事件处理程序(见[清单 9-7](#list_9_7) )。
...
不管用户是否完成了拖放，我们都需要一个 dragend 处理程序来清理(见[清单 9-12](#list_9_12) )。
