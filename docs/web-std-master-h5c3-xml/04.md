...
现在，当服务器接收到对不存在img/logo`img/logo`的请求时，服务器在`images`目录中搜索所有名为`logo.*`的文件，根据每个文件的扩展名分配 MIME 类型。然后，服务器根据偏好选择最佳匹配，并交付该资源。例如，让我们假设`images`目录包含以下文件变量:`logo.svg`、`logo.png`和`logo.gif`。当有一个针img/logo`的查询时，优先顺序将被考虑在对该查询的回答中([清单 4-17](#list_4_17) )。
