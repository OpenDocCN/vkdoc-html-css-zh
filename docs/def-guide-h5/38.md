...
`code`属性有三个可能的值。这些属性在[表 **38-** 6](#tab_38_6) 中描述。
...
![Image](img/square.jpg) **警告**主流浏览器的当前版本没有很好地实现`watchPosition`方法，更新的位置也不总是即将到来。使用定时器(我在第 27 章的[中描述过)并定期调用`getCurrentPosition`方法可能会更好。](27.html#ch27)
