...
Web 的现有网络架构也将补充 WebGL。WebGL 应用将能够从 URL 获取纹理和模型等资源。多人游戏可以用 WebSocket 通信。例如，[图 13-1](#fig_13_1) 显示了一个这样的例子。谷歌最近使用 HTML5 WebSocket、Audio 和 WebGL 将经典的 3D 游戏 Quake II 移植到了网络上，并完成了多人游戏。游戏逻辑和图形用 JavaScript 实现，调用 WebGL 画布进行渲染。使用持久的 WebSocket 连接来实现与服务器的通信，以协调玩家的移动。
