...
![image](images/square.jpg) **注意** CSS3 和 HTML5 互不依赖；它们是完全不同的建议。
...
![image](images/square.jpg) **提示**安装一个文本编辑器，比如 Notepad ++，它比 MS Notepad 有几个增强。

当一段 CSS 标记不能包含在一行中时，使用一个向左的箭头来表示这两行属于同一行；例如:

`#header { width:920px; height:180px; padding:0; margin: 20px auto; ![image](images/U002.jpg)
border:10px white solid; **background: url(images/**h**eader3.jpg) no-repeat**;
        }`
...
![image](images/square.jpg) **提示**参见附录中的表格，了解浏览器在美国、英国和欧洲的使用情况。如果你的目标是一个特定的区域，这是很有用的。