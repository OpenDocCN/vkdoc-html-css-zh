...
图 4-1 显示了 TCP 上互联网应用层协议的典型分层。应用使用 XMPP 或 STOMP (简单的面向文本的消息协议T5，我们将在[第 5 章](05.html)中讨论)之类的协议在客户端和服务器之间进行通信。XMPP 和 STOMP 依次通过 TCP 进行通信。使用加密通信时，应用层协议位于 TLS(或 SSL)之上，而 TLS 又位于 TCP 之上。
