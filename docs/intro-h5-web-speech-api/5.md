...
然后，在我们添加一个动态生成的水平规则元素以将其与下一个评审反馈分开之前，使用`review.appendChild(s)`将这些内容和抄本的内容一起添加到 DOM 中的评审区域。在第三个也是最后一个事件处理程序中，我们使用`clearbtn`来触发清空脚本文本区域的内容，这样就可以准备好记录下一个评论了。
