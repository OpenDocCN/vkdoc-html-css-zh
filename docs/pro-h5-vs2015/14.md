...
在[第 10 章](10.html)中，你用`canvas`元素画了一个棋盘。然而，这对于这个应用来说是行不通的，因为每个方块都需要单独的 DOM 元素。您可能会尝试使用 SVG 来创建棋盘，因为每个`rect`元素都是一个独立的 DOM 元素；然而，SVG 元素不支持 DnD API。
...
试试这个应用，当你移动棋子时，你应该会看到如图 [14-6](#Fig6) 所示的笑脸。
