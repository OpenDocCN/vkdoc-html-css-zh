...
浏览器将下载清单中指定的所有内容，即使这些内容尚未被要求。对于我们的简单应用，这意味着我们的三个图像都被下载了。现在应用继续正常工作，甚至当我离线时，如[图 40-4](#fig_40_4) 所示。
...
我已经从缓存部分删除了`cherries100.png`,这样我们就有了应用需要但不可用的资源。你可以在[图 40-5](#fig_40_5) 中看到浏览器如何处理回退。

![Image](img/4005.jpg)

*图 40-5。使用回退内容*

`offline.png`图像是一个简单的十字。为图像提供后备并不理想，但至少我们可以维护页面的结构和布局。回退对于链接到其他文档可能更有用。[清单 40-6](#list_40_6) 显示了我们的 web 应用文档的一个变更，它包含了一个到另一个 HTML 文件的链接。
