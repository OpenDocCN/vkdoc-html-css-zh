...
在 CQL 中，`RETURN`子句与`MATCH`命令一起使用，从 Neo4j 图数据库中检索关于节点、关系和属性的数据。`RETURN`子句可以检索节点的部分或全部属性，并检索节点的部分或全部属性以及关联关系。`RETURN`子句的参数是节点名和属性名(参见清单 [7-39](#FPar46) )。
