...
当您在移动浏览器中打开此文档时，您会注意到浏览器中的任何内容都已被设置为设备的宽度。对于苹果 iPhone，这将是 320 CSS 像素宽；Kindle Fire 的像素是 600 CSS。这种可变宽度是一个很好的特性。由于它使浏览器看起来像是设备的原生应用程序，这是广告应该考虑的事情，因为大多数发布者页面都会包含这些元标签。清单 8-4 通过设置比例值和用户输入显示了一个更精细的视口示例。
