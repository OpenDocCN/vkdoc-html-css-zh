...
PrimeFaces 有 100 多个 UI 组件，可以用来创建丰富的 Web 2.0 应用。深入这些组件的细节超出了本书的范围。然而，我们将在[表 9-2](#Tab2) 中给出这些组件的简短列表供您参考，根据 PrimeFaces 文档按字母顺序排序。
...
Country Navigator 应用是一个 PrimeFaces 应用，它允许用户在点击任何一个可用国家的旗帜后，获得一个可用国家的可用城市列表(带有一些信息)，如图[图 9-1](#Fig1) 所示。
...
[清单 9-6](#list6) 显示了 Country Navigator 应用页面的完整代码，它将 ring 组件与“details”面板部分合并在一起。
...
RichFaces 有 40 多个 UI 组件，可以用来创建丰富的 Web 2.0 应用。深入这些组件的细节超出了本书的范围。但是，我们将在[表 9-3](#Tab3) 中给出一些组件的简短列表，供您参考，根据 RichFaces 文档按字母顺序排序(注意，所有这些组件都是 rich 标签库的一部分)。
...
最后，[清单 9-15](#list15) 展示了 RightCountry 应用的 CSS 样式类。
