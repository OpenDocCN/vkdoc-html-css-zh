...
![9781484204610_Fig04-06.jpg](../Images/image00384.jpeg)
...
![9781484204610_Fig04-09.jpg](../Images/image00387.jpeg)
...
![9781484204610_Fig04-10.jpg](../Images/image00388.jpeg)
...
[图 4-13](#_Fig13) 。默认激活的<轨道>元素的 IDL 属性
...
[图 4-15](#Fig15) 显示了<track>元素加载前`@track`属性中`TextTrack`对象的值，以及在 Google Chrome 中加载后的提示数。您会看到 `@cues`的长度是 0(因为“cues”在加载前为空，而在加载后为 51)。
...
加载视频后，我们创建一个 vtt cue—`new VTTCue`—它是一种`TextTrackCue`。我们从 51 个线索开始，到 52 个结束。开始回放后，我们加载了所有的 52 个线索，然后注册了一个`cuechange`事件，在这个事件中，线索 1 被删除，返回到 51 个线索。[图 4-16](#Fig16) 显示了谷歌浏览器中的结果。还要注意，第一个提示是 52 个提示列表中的脚本创建的提示。
...
![9781484204610_Fig04-17.jpg](../Images/image00395.jpeg)
...
在[清单 4-14](#list14) 中，我们有一个通过<轨道>元素创建的字幕轨道和一个脚本创建的章节轨道。
...
`*   `Voice span <v>`: to mark up a section of text with a voice and speaker annotation, for example,
 ...
 `![Image](../Images/image00312.jpeg) **注意**除非另行通知，否则您需要使用< c >标签将文本括在时间戳之间，以使 CSS 选择器生效(直到`www.w3.org/Bugs/Public/show_bug.cgi?id=16875`被解析)。
...
![9781484204610_Fig04-19.jpg](../Images/image00398.jpeg)
...
![Image](../Images/image00312.jpeg) **注意**小心文本定位，因为提示的最终位置取决于文本的对齐方式。例如，如果提示居中对齐，文本位置的原点将是文本块的中心。对于右对齐文本，它将是块的右边缘，依此类推。如果得到不一致的结果，首先要查看提示对齐属性。
...
为了更好地理解所有这些提示设置的效果，[清单 4-17](#list17) 显示了一个使用线条、对齐、位置和大小设置来适应变化的提示位置和宽度的提示示例。

[***清单 4-17***](#_list17) 。带有提示的 WebVTT 文件，带有提示设置
...
[图 4-21](#Fig21) 显示了 Chrome(左)和 Safari(右)中的结果。

![9781484204610_Fig04-21.jpg](../Images/image00401.jpeg)

[图 4-21](#_Fig21) 。在 Chrome 和 Safari 中使用提示设置渲染提示
...
对于 WebM 文件，你可以使用`mkvmerge`(参见 GUI 应用程序的`http://mkvtoolnix.en.softonic.com/`)。下面是一个例子:
...
我们正试图提取[清单 4-19](#list19) 中的`@videoTracks`和`@audioTracks`属性的内容，这样我们也许能够操纵哪个音频或视频轨道是活动的。然而，[图 4-22](#Fig22) 显示我们并不幸运——Safari 只显示 0 个视频和音频轨道。

![9781484204610_Fig04-22.jpg](../Images/image00402.jpeg)

[图 4-22](#_Fig22) 。在 Safari 中渲染@videoTracks 和@ audio tracks
...
[***清单 4-19***](#_list19) 。将主视频和手语视频捆绑在一起
...
![9781484204610_Fig04-23.jpg](../Images/image00403.jpeg)

[图 4-23](#_Fig23) 。Safari 中从属媒体元素的呈现
...
[***清单 4-20***](#_list20) 。使用 MediaController 将主视频和音频描述捆绑在一起
...
![9781484204610_Fig04-24.jpg](../Images/image00404.jpeg)

[图 4-24](#_Fig24) 。Safari 中的 MediaController 对象及其从属元素
...
[***清单 4-21***](#_list21) 。使用媒体片段 URIs 导航章节
...
导航到“Emo 创建”章节后，结果可在[图 4-25](#Fig25) 中看到。

![9781484204610_Fig04-25.jpg](../Images/image00405.jpeg)

[图 4-25](#_Fig25) 。使用谷歌浏览器中的媒体片段 URIs 导航章节
